...
class Locker:
	...
	def ball_alg1(num_lockers, num_keys, num_balls, given_keys, desired_lockers):
		...
		for keys in chain.from_interable(combinations(key_set, r) for r in range(len(key_set)+1)):
			not_opened = 0
			keys = sorted(keys)
	...
	def file_load(self, file, idx):
		...
		self.tennis_balls = [int(num) for num in tennis_balls.split(' ')]
		self.keys = [int(num) for num in keys.split(' ')]
